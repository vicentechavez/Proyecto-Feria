...
class DeteccionMovimientoApp:
    ...
    def modificar_datos_persona(self):
        selected_item = self.tree_personas_admin.selection()
        if not selected_item:
            messagebox.showwarning("Sin Selección", "Por favor, selecciona una persona para editar.", parent=self.ventana_admin)
            return

        item_values = self.tree_personas_admin.item(selected_item, 'values')
        id_imagen, nombre, rut, fecha_nac_str, _, relacion, _ = item_values

        ventana_editar = tk.Toplevel(self.ventana_admin)
        ventana_editar.title(f"Editando a {nombre}")
        ventana_editar.geometry("400x380")
        ventana_editar.configure(bg='#4d4d4d')
        ventana_editar.transient(self.ventana_admin)
        ventana_editar.grab_set()
        self.centrar_ventana(ventana_editar, self.ventana_admin)

        entradas = {}

        ttk.Label(ventana_editar, text="Nombre Completo:", style='Black.TLabel').pack(pady=(10, 2))
        entrada_nombre = ttk.Entry(ventana_editar, style='Black.TEntry')
        entrada_nombre.pack(pady=2, padx=20, fill='x')
        entrada_nombre.insert(0, nombre)
        entradas["nombre_completo"] = entrada_nombre

        ttk.Label(ventana_editar, text=f"RUT: {rut} (No editable)", style='Black.TLabel').pack(pady=(10, 2))

        ttk.Label(ventana_editar, text="Fecha de Nacimiento:", style='Black.TLabel').pack(pady=(10, 2))
        frame_fecha = ttk.Frame(ventana_editar, style='Black.TFrame')
        ...
        combo_dia = ttk.Combobox(frame_fecha, values=[f"{i:02d}" for i in range(1, 32)], width=5, state="readonly", style='Black.TCombobox')
        combo_dia.set(dia_actual)
        combo_dia.pack(side=tk.LEFT, padx=2, expand=True)
        entradas["dia_nacimiento"] = combo_dia

        combo_mes = ttk.Combobox(frame_fecha, values=[f"{i:02d}" for i in range(1, 13)], width=5, state="readonly", style='Black.TCombobox')
        combo_mes.set(mes_actual)
        combo_mes.pack(side=tk.LEFT, padx=2, expand=True)
        entradas["mes_nacimiento"] = combo_mes

        current_year = datetime.now().year
        combo_anio = ttk.Combobox(frame_fecha, values=[str(i) for i in range(current_year, current_year - 100, -1)], width=7, state="readonly", style='Black.TCombobox')
        combo_anio.set(anio_actual)
        combo_anio.pack(side=tk.LEFT, padx=2, expand=True)
        entradas["anio_nacimiento"] = combo_anio

        ttk.Label(ventana_editar, text="Relación:", style='Black.TLabel').pack(pady=(10, 2))
        combo_relacion = ttk.Combobox(ventana_editar, values=["Dueño", "Hijo", "Esposa", "Empleado", "Visita", "Otro"], state="readonly", style='Black.TCombobox')
        combo_relacion.set(relacion)
        combo_relacion.pack(pady=2, padx=20, fill='x')
        entradas["relacion"] = combo_relacion

        btn_guardar = ttk.Button(ventana_editar, text="Guardar Cambios", style='TButton',
                                 command=lambda: self._ejecutar_actualizacion_persona(id_imagen, ventana_editar, entradas))
        btn_guardar.pack(pady=20)

    def _ejecutar_actualizacion_persona(self, id_imagen, ventana_editar, entradas):
        try:
            # 1. Recoger y validar datos de la ventana
            ...
            sql = """
                ...
                SET nombre_completo = %s, fecha_nacimiento = %s, edad = %s, relacion = %s
                WHERE id_imagen = %s
            """
            cursor.execute(sql, (nombre_nuevo, fecha_nac_dt_nueva.strftime('%Y-%m-%d'), edad_nueva, relacion_nueva, id_imagen))
            self.conexion.commit()

            # 4. Refrescar datos y notificar
            self._update_system_status() # Notifica a otros clientes del cambio
            self._reload_data_from_db()  # Recarga los datos en este cliente
            self._cargar_personas_admin_treeview() # Refresca la tabla del panel de admin

            ventana_editar.destroy()
            messagebox.showinfo("Éxito", "Los datos de la persona han sido actualizados correctamente.", parent=self.ventana_admin)

        except ValueError as e:
            messagebox.showerror("Error de Datos", str(e), parent=ventana_editar)
        except mysql.connector.Error as err:
            messagebox.showerror("Error de Base de Datos", f"No se pudo actualizar el registro: {err}", parent=ventana_editar)
            self.conexion.rollback()
        finally:
