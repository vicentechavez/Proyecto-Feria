...
class DeteccionMovimientoApp:
    ...
    def mostrar_datos_con_imagenes(self):
        self.cursor.execute("SELECT nombre, imagen FROM reconocimiento")
        resultados = self.cursor.fetchall()

        ventana = tk.Toplevel(self.root)
        ventana.title("Datos en la Base de Datos con Imágenes")
        ...
        self.imagenes_mostradas = []  # Para mantener referencias a las imágenes

        if resultados:
            for nombre, imagen_blob in resultados:
                # Convertir blob a imagen PIL
                imagen_pil = Image.open(BytesIO(imagen_blob))
                imagen_pil = imagen_pil.resize((100, 100))  # ajustar tamaño

                imagen_tk = ImageTk.PhotoImage(imagen_pil)
                self.imagenes_mostradas.append(imagen_tk)  # evitar que se borre por GC

                frame_item = ttk.Frame(scrollable_frame, padding=5, relief="ridge")
                frame_item.pack(fill="x", pady=5)

                label_imagen = ttk.Label(frame_item, image=imagen_tk)
                label_imagen.pack(side="left")

                label_nombre = ttk.Label(frame_item, text=nombre, font=("Helvetica Neue", 12))
